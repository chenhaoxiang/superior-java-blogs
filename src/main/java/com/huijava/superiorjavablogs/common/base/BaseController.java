...
public class BaseController {
     ...
     * @param model
     */
    protected void loagTabbableBlogsLost(Model model, BlogsService blogsService) {
        List<BlogsDTO> viewBlogsList = blogsService.selectBlogsDTOByDaysAndNumber(180, 10, BlogFieldEnum.VIEW_COUNT);
        log.debug("获取访问量排行的文章:viewBlogsList.size={}", viewBlogsList.size());
        model.addAttribute("viewBlogsList", viewBlogsList);
        //获取点赞数排行的文章 - 180日内 16篇文章
        List<BlogsDTO> topBlogsList = blogsService.selectBlogsDTOByDaysAndNumber(180, 10, BlogFieldEnum.TOP_COUNT);
        log.debug("获取点赞数排行的文章:topBlogsList.size={}", topBlogsList.size());
        model.addAttribute("topBlogsList", topBlogsList);
    }
     ...
     * @return
     */
    protected Users getLoginUsers(HttpServletRequest request) {
        return (Users) request.getSession().getAttribute(UserConstans.LOGIN_SESSION_NAME);
    }
